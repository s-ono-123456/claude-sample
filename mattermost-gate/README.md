...
Claude Code は自律的にファイル編集・シェルコマンド実行などのツールを使用します。
本番環境や共有環境では、すべての操作を自動承認するのは危険です。

このツールは Claude Code の `PermissionRequest` フックとして動作し、
ツール実行のたびに Mattermost へ通知を投稿します。
オペレーターが ✅ / ❌ のリアクションで承認・拒否することで、
**Claude Code の操作を人間が制御下に置くゲートウェイ**を実現します。
...
# config.json を開き mattermost_url / token / channel_id を設定

# 3. Claude Code のフック登録（後述「セットアップ詳細 > Claude Code フック登録」を参照）

# 4. 動作確認
'{"tool_name":"Bash","tool_input":{"command":"ls","description":"test"},"cwd":"C:\\claude","session_id":"test"}' | .venv\Scripts\python.exe mattermost-gate\permissionrequest.py
```
...
`.claude/settings.json` に以下の設定を追記します。
...
**`token` が無効でリクエストが失敗する**
Mattermost の System Console からトークンを再発行し、`config.json` の `token` を更新してください。

**`channel_id` の取得方法がわからない**
対象チャンネルを Mattermost ブラウザで開き、URL の最後のパス部分（例: `abc123def456`）が channel_id です。
または Mattermost API の `GET /api/v4/channels/name/{team_name}/{channel_name}` で取得できます。

**承認前にタイムアウトしてしまう**
`config.json` の `timeout_seconds` を大きくしてください（デフォルト: `300` 秒）。
